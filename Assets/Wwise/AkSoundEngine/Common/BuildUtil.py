...
# //
# //////////////////////////////////////////////////////////////////////
import sys, os, os.path, shutil, subprocess, logging, logging.handlers, argparse, platform, json
...
SupportedPlatforms = \
{
	'Windows': ['Windows', 'Android', 'Metro', 'PS3', 'XBox360', 'Vita'],
...
PlatformSwitches = \
{
	'Windows': '#if UNITY_STANDALONE_WIN || (UNITY_EDITOR_WIN && UNITY_STANDALONE_WIN) || (UNITY_EDITOR_WIN && UNITY_XBOX360) || (UNITY_EDITOR_WIN && UNITY_PS3) || (UNITY_EDITOR_WIN && UNITY_ANDROID) || (UNITY_EDITOR_WIN && UNITY_METRO) || (UNITY_EDITOR_WIN && UNITY_PSP2)',
...
def RunCommand(cmd):
	...
	if IsPython3:
		stdOut = stdOut.decode(DefaultCodec)
		stdErr = stdErr.decode(DefaultCodec)
...
def ExportFile(outputFile, outputLines):
	...
	for ll in range(len(outputLines)):
		hasNoLinebreak = outputLines[ll].find(os.linesep) == -1 and outputLines[ll].find(SpecialChars['PosixLineEnd']) == -1
		if hasNoLinebreak:
			outputLines[ll] += SpecialChars['PosixLineEnd']
