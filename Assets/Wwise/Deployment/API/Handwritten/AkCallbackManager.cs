...
static public class AkCallbackManager
{
    ...
    static public void PostCallbacks()
    {
        ...
        bool isValidEventCallback = m_mapEventCallbacks.TryGetValue((int)commonCB.pPackage, out eventPkg);
        if ( ! isValidEventCallback )
        {
            bool isValidBankCallback = m_mapBankCallbacks.TryGetValue((int)commonCB.pPackage, out bankPkg);
            if ( ! isValidBankCallback )
            {
                AkCallbackSerializer.Unlock();
                return;
            }
        ...
        do
        {
            ...
            else if (commonCB.eType == AkCallbackType.AK_Bank)
            {
                ...
                GotoEndOfCurrentStructMemberOfValueType<IntPtr>(ref pData);
            ...
            eventPkg = null;
            isValidEventCallback = m_mapEventCallbacks.TryGetValue((int)commonCB.pPackage, out eventPkg);
            if ( ! isValidEventCallback )
            {
                bool isValidBankCallback = m_mapBankCallbacks.TryGetValue((int)commonCB.pPackage, out bankPkg);
                if ( ! isValidBankCallback )
